...
<div class="row-offcanvas row-offcanvas-left">
 ...
 <div class="row">
         ...
         <div class="row">
             ...
             <div class="Right_h" align="center"><b>Tracking Request</b></div><hr/>
            ...
            <?php
			   include('hsd/dibba_laga.php');		
								...
								$dalia = mysql_query("SELECT * FROM friends WHERE requested='".$_SESSION['SESS_MEMBER_ID'] ."' and status='pending' ORDER BY requested ASC");
								
								
								while($tuhan = mysql_fetch_array($dalia))
								  {
								  $hami=$tuhan['addedby'];
								  $dalia1 = mysql_query("SELECT * FROM members WHERE user='$hami'");
			  ...
			  echo '<div style=" margin-right:3px; float:left">';
			  		echo '<img src="http://localhost/social/'.$tuhan1['profilepic'].'" height="30" width="30">';
					echo '</div>';
					echo '<div style="margin-top:-5px; float:right">';
					 echo  '<a href=http://localhost/social/user/'.$tuhan['addedby'] .'><strong>'.$tuhan1['fname'].' '.$tuhan1['lname'].'</strong></a><br/>';
					  echo  '<font size="2px" color="#b621ff">'.$tuhan1['city'].'</font>&nbsp;|&nbsp;<font size="2px" color="#27cefd">'. $tuhan1['country'].'</font>';
					
					echo '</div>';
				echo '</div>';
				echo '<div style="float:right">';	
					echo '<a href=http://localhost/social/accept/'.$tuhan["id"] .'&amp;'.'uid='.$tuhan["addedby"].' title="Confirm request">'.'Track me'.'</a>';
					echo ' | ';
					echo '<a rel="facebox" href=http://localhost/social/tracker/list/out/'.$tuhan["id"] .' title="Delete request">'.'No way!'.'</a>';
					echo '</div>';
			   echo '</div>';
			   echo '<br><br><br/>';
